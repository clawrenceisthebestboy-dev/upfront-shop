...
class JobEditor(QDialog):
    ...
    def _snapshot(self, doc_kind: str):
        ...
        # If the user hasn't explicitly picked a logo, fall back to the
        # bundled / assets-dir copy so every invoice carries the branding.
        shop["logo_path"] = db.resolve_logo_path(self.conn)
        render_invoice_pdf(
            str(out),
            shop=shop,
