...
def default_db_path() -> Path:
    ...
    Windows: %APPDATA%\\UpFrontShop\\shop.db
    macOS:   ~/Library/Application Support/UpFrontShop/shop.db
    Linux:   ~/.upfront-shop/shop.db
    """
    if os.name == "nt":
        base = Path(os.environ.get("APPDATA", str(Path.home()))) / "UpFrontShop"
    elif sys.platform == "darwin":
        base = Path.home() / "Library" / "Application Support" / "UpFrontShop"
    else:
...
def assets_dir() -> Path:
    """Where user-customised assets (logo, etc.) live alongside the DB."""
    d = default_db_path().parent / "assets"
    d.mkdir(parents=True, exist_ok=True)
    return d


def bundled_resource_dir() -> Path:
    """Read-only bundled resources shipped inside the app folder."""
    # When frozen (PyInstaller) the resources are next to the executable;
    # in dev, they're in the repo checkout.
    here = Path(__file__).resolve().parent.parent
    for p in (here / "resources", here.parent / "resources"):
        if p.is_dir():
            return p
    return here / "resources"


def resolve_logo_path(conn: sqlite3.Connection) -> str:
    """Return a usable logo path, in this priority order:
      1. Whatever the user picked in Settings -> Branding / QR.
      2. A user-installed 'logo.*' file in the assets_dir (handy if Josh
         copies the file in manually without going through the UI).
      3. A bundled 'upfront_logo.*' shipped alongside the app.
      4. The placeholder_logo.png (so the layout never breaks).
      5. Empty string — print text-only header.
    """
    pref = get_setting(conn, "logo_path", "").strip()
    ...
    for name in ("logo.png", "logo.jpg", "logo.jpeg", "logo.webp"):
        ...
        if p.is_file():
            return str(p)
    for name in ("upfront_logo.png", "upfront_logo.jpg", "upfront_logo.jpeg"):
        p = bundled_resource_dir() / name
        if p.is_file():
            return str(p)
    placeholder = bundled_resource_dir() / "placeholder_logo.png"
